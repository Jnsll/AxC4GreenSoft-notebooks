...
def _load_data():
    messages = pd.read_csv(
        file + '/dataset/SMSSpamCollection',
        sep='\t',
        names=['label', 'message']
    )
    return messages
...
def _preprocess(messages):
    ...
    dump(preprocessor, file + '/output/preprocessor.joblib')
    dump(preprocessed_data, file + '/output/preprocessed_data.joblib')
    return preprocessed_data

def prepare(message):
    preprocessor = load(file + '/output/preprocessor.joblib')
